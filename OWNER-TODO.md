...
| **Consent versions + copy** (terms/privacy/analytics/marketing) | The consent API (`POST /auth/consent`) stores a `version` string per capture (I6 Slice 7). Set the real current version strings + the consent UI copy when the legal text lands; the account-deletion + export flows already honor them. | production | Mechanism built; version strings are owner-supplied. |
