...
2. **No real ad-serving or payment** (blueprint: parked). Build the **placement/inventory structure** + a **dormant Stripe seam**: **Mock** marks an order paid without charging; **Live** stays dormant until a key + `APP_MODE`. **All pricing in `app_settings`** (configurable; rises with demand). No card data ever touches our DB.
...
The revenue dashboard over a real slot model. **`ad_slots`** (configurable records: key, page/context, format, default unsold-fallback = hide|organic, active) — every `AdSlot` on the site references a slot key. **`ad_placements`/orders** (slot, advertiser label + contact, **UGC creative: label/link/text — escaped + validated**, schedule from/to, sold-state, price from `app_settings`, status draft/scheduled/active/ended). **Inventory view** (every slot incl. each game page, sold/free/scheduled), **free-inventory view**, **per-slot analytics** (impressions on render + clicks, aggregate; occupancy). **"Promote a game" flow**: public "Promote your game" form → email to us (mailto, like Contact) → admin creates a placement → a **labeled Promoted flag** on the game (reuses the ad-slot mechanism; payment is the filter, we don't verify the submitter). **Dormant Stripe seam** + order model: Mock marks paid without charging; Live dormant. **Every paid placement renders a Promoted/Sponsored label as prominent as a bias flag.**
...
- **Prove-the-attack-fails `verify:i8`** (grows slice by slice): RBAC on the Control Panel (a moderator can't reach admin/owner sections; a non-staff/anon can't reach the panel at all); **placement creative is stored raw + rendered escaped** (UGC injection); **paid placements always carry the Promoted label** (transparency can't be toggled off); the **payment seam never charges in demo** and no secret is in code/logs; newsletter **segmentation excludes non-consented** addresses and **Mock send writes only to the outbox** (zero network); dashboard/analytics expose **no per-user data**; audit rows for every staff action.
